...
/*
let gk = "Neuer"
const fieldPlayers = ["Davies1","Davies2","Davies3","Davies4","Davies5","Davies6","Davies7","Davies8","Davies9","Davies10",]
...
for(const [index, player] of game.scored.entries()){
  console.log(`player ${player} scored goal number ${index +1 }`);
}
...
console.log(cumul/oddsList.length)

//3
for(const [teamName, odds] of Object.entries(game.odds)){
  console.log(`Odd of victory ${game[teamName] ?? 'draw'}: ${odds}`)
};
...
for(const players of game.scored){
  let curr = scorers[players] ?? 0
  scorers[players] = curr + 1;
}
console.log(scorers)
...
function convertToCamelCase(){
  ...
  for(const [i, word] of wordarray.entries()){
    ...
    console.log([first,second].join("").padEnd(25," ") + "✅".repeat(i+1));
  }
